...
#include <object_detector/Detect.h>
...
std::atomic<double> msgX;
std::atomic<double> msgY;
std::atomic_int class_num;
...
// 接收tensorflow转换
void boxCallback(object_detector::Detect detect)
{
    object_detector::Box box = detect.boxes[0];
    class_num = detect.classes[0];
    msgX = box.box[0];
    msgY = box.box[1];
    
    OUT_INFO("get msg: (" + std::to_string(msgX) + ", " +
            std::to_string(msgY) + ")");
    OUT_INFO("get type: " + std::to_string(class_num));
}
...
class Receiver
{
    ...
    private:
        void start(const Mode mode)
        {
            ...
            rightBtnPointPub = nh.advertise<geometry_msgs::PointStamped>("/kinect2/click_point/right", 1);
        ...
        void cloudViewer() {
            ...
            ros::Subscriber sub = nh.subscribe("/image_objects_detect/box", 10, boxCallback);
            ...
            for(; running && ros::ok();) {
                if(updateCloud) {
                    ...
                    //         OUT_INFO("mousePosition: " + std::to_string(pt.x) + ", " + std::to_string((pt.y)));
                    //         ros::spinOnce();
                    //         break;
